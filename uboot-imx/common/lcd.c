...
#if LCD_BPP == LCD_COLOR32
	if (bpix != bmp_bpix && (bmp_bpix != 8 || bpix != 32)) {
#else
...
#if LCD_BPP == LCD_COLOR32
			ulong colreg =
				(cte.red << 16) | (cte.green << 8) | cte.blue ;
#else
			ushort colreg =
				( ((cte.red)   << 8) & 0xf800) |
				( ((cte.green) << 3) & 0x07e0) |
...
//	sp = (unsigned short *)lcd_base;
//	for(i = 0; i < panel_info.vl_row * panel_info.vl_col;i++)
//	{
//		*sp++ = 0xffe0;
//	}
