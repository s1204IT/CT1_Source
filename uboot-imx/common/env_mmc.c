...
int saveserialmac(const unsigned char* ptr)
{
	...
	printf("Writing to MMC(%d)... ",devno);
	if (write_env_devno(mmc, CONFIG_SERIAL_SIZE, CONFIG_SERIAL_OFFSET, ptr,devno)) {
		puts("failed\n");
		return 1;
	}
...
int read_serialmac(unsigned char* pbuff)
{
	...
	if (read_env_devno(mmc, CONFIG_SERIAL_SIZE, CONFIG_SERIAL_OFFSET, pbuff,devno))
		return 1;
