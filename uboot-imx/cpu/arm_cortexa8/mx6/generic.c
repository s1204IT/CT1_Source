...
#ifdef CONFIG_MX6DL_BEE01
#define ANDROID_REBOOT_BOOT  (1 << 8)
int reboot_check_and_clean_flag(void)
{
	int flag_set = 0;
	u32 reg;
	...
	flag_set = !!(reg & ANDROID_REBOOT_BOOT);

	/* clean it in case looping infinite here.... */
	if (flag_set) {
		reg &= ~ANDROID_REBOOT_BOOT;
		writel(reg, SNVS_BASE_ADDR + SNVS_LPGPR);
	}
...
#define BATTERY_LOADMODEL_OPERATION  (1 << 9)
void set_loadmodel_operation_flag(int value)
{
	u32 reg;
	...
	if(value)
		reg |= BATTERY_LOADMODEL_OPERATION;
	else
		reg &= ~BATTERY_LOADMODEL_OPERATION;
...
int get_loadmodel_operation_flag(void)
{
	u32 reg;
	int ret;
	
	reg = readl(SNVS_BASE_ADDR + SNVS_LPGPR);
	ret = reg & BATTERY_LOADMODEL_OPERATION;
...
#if defined CONFIG_MX6DL_BEE01 || defined CONFIG_MX6DL_BEH01
#define PERSIST_WATCHDOG_RESET_BOOT		(0x10000000)
#define BOOT_MODE_SDHC3					(0x00003040)
 ...
 * watchdog reset will enter mfg download mode again, clear this bit
 * to prevent this behavior */
void clear_sdbootmode_mem(void)
{
	u32 reg;
...
#if defined CONFIG_SDTEST || defined CONFIG_SDWRITE
int chk_sdbootmode(void)
{
	u32 reg;

	reg = readl(SRC_BASE_ADDR + SRC_GPR9);
	if((reg & BOOT_MODE_SDHC3) == BOOT_MODE_SDHC3)
	{
		return 1;
	}
