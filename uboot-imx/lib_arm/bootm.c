...
void do_booti_linux (boot_img_hdr *hdr)
{
	...
	char serialno[16];
	char macaddr[18];
	char *autorun = getenv("autorun");
#ifdef CONFIG_SDTEST
	char *testkey = getenv("testkey");
#endif
...
#else
	if (!commandline)
		strcpy (buff, (char *)hdr->cmdline);
	else
		strcpy (buff, (char *)commandline);
	...
	if(!serialno_mac_read(serialno,macaddr))
	{
	...
	strncat(buff," bootVer=",sizeof(buff));
	strncat(buff,CONFIG_BEE01_BOOT_VER,sizeof(buff));
