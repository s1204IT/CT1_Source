...
#ifdef CONFIG_TOUCHSCREEN_BU21025
...
u32 calibration_voltage(struct max8903_data *data)
{
	...
	int i;
	for (i = 0; i < ADC_SAMPLE_COUNT; i++) {
		volt[i] = (rohm_bu21025gul_read_adc()-1230)*(1000000/380)+3300000;
	}
	sort(volt, i, 4, cmp_func, NULL);
	for (i = 0; i < ADC_SAMPLE_COUNT; i++)
		pr_debug("volt_sorted[%2d]: %d\n", i, volt[i]);
...
#elif  CONFIG_TOUCHSCREEN_BU21025
		temp = calibration_voltage(data);
//printk("ozaki-test:max8903_battery_update_status  temp:%d\n", temp);
#else
		temp = 4000000;
#endif
		if (temp_last == 0) {
