...
#define I2C_BASE CONFIG_I2C1_PORT
...
void i2c1_init(int speed, int unused)
{
	int freq;
	int i;
...
int i2c1_read(uchar chip, uint addr, int alen, uchar *buf, int len)
{
	int timeout = I2C_MAX_TIMEOUT;
	uint ret;
...
int i2c1_write(uchar chip, uint addr, int alen, uchar *buf, int len)
{
	int timeout = I2C_MAX_TIMEOUT;
	DPRINTF("%s chip: 0x%02x addr: 0x%04x alen: %d len: %d\n",
		__func__, chip, addr, alen, len);
