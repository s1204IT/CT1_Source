...
static int mxcfb_register(struct fb_info *fbi)
{
	...
	if (ipu_request_irq_ex(mxcfbi->ipu, mxcfbi->ipu_ch_irq,
		mxcfb_irq_handler, IPU_IRQF_ONESHOT, MXCFB_NAME, fbi, false) != 0) {
		dev_err(fbi->device, "Error registering EOF irq handler.\n");
		ret = -EBUSY;
		goto err0;
	}
	ipu_disable_irq(mxcfbi->ipu, mxcfbi->ipu_ch_irq);
	if (ipu_request_irq_ex(mxcfbi->ipu, mxcfbi->ipu_ch_nf_irq,
		mxcfb_nf_irq_handler, IPU_IRQF_ONESHOT, MXCFB_NAME, fbi, false) != 0) {
	...
	if (mxcfbi->ipu_vsync_pre_irq != -1) {
		if (ipu_request_irq_ex(mxcfbi->ipu, mxcfbi->ipu_vsync_pre_irq,
				    mxcfb_vsync_pre_irq_handler, 0,
				    MXCFB_NAME, fbi, false) != 0) {
...
extern void bee01_backlight_enable(void);
#endif
