...
int ipu_request_irq_ex(struct ipu_soc *ipu, uint32_t irq,
		    irqreturn_t(*handler) (int, void *),
			uint32_t irq_flags, const char *devname, void *dev_id, bool enable_irq)
{
	uint32_t reg;
	unsigned long lock_flags;
...
EXPORT_SYMBOL(ipu_request_irq_ex);
