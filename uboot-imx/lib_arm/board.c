...
#endif
#ifdef CONFIG_SDTEST
	if ((s = getenv ("ddrfreq")) != NULL) {
		uchar ddrclockcmd[20];

		sprintf(ddrclockcmd, "ddrclk %s", s);
		if(strcmp(ddrclockcmd,"ddrclk 396") != NULL)
		{
			run_command(ddrclockcmd,0);
		}
	}
	else {
		run_command("ddrclk 452",0);
	}
#endif
