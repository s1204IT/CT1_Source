...
void do_switch_loadmodel_operation(int value)
{
	u32 reg;
	void __iomem *ioaddr;
	
	ioaddr = ioremap(MX6Q_SNVS_BASE_ADDR, 0xC00);

	reg = __raw_readl(ioaddr + SNVS_LPGPR);
	if(value)
		reg |= BATTERY_LOADMODEL_OPERATION;
	else
		reg &= ~BATTERY_LOADMODEL_OPERATION;
	__raw_writel(reg, ioaddr + SNVS_LPGPR);
}

int get_loadmodel_operation_flg(void)
{
	u32 reg;
	int ret;
	void __iomem *ioaddr;
